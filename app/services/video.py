...
def add_pen_pop_sound(video_path: str, sound_path: str, at_seconds: float = 3.0) -> str:
    """Mix a pen pop sound into the video at a specific time."""
    output_path = video_path.replace(".mp4", "_with_pop.mp4")
    
    cmd = [
        "ffmpeg", "-y",
        "-i", video_path,           # input video with audio
        "-i", sound_path,           # pen pop sound
        "-filter_complex",
        f"[1:a]adelay={int(at_seconds * 1000)}|{int(at_seconds * 1000)}[delayed];"
        f"[0:a][delayed]amix=inputs=2:duration=first:dropout_transition=0[aout]",
        "-map", "0:v",
        "-map", "[aout]",
        "-c:v", "copy",
        "-c:a", "aac",
...
def generate_video(
    ...
    # Add pen pop sound at 3 seconds
    sound_file = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "resource", "sounds", "pen_pop.mp3")
    if os.path.exists(sound_file):
        add_pen_pop_sound(str(output_file), sound_file, at_seconds=3.0)
        logger.info("pen pop sound added at 3 seconds")
